...
public class SharingToFacebook {
	...
	public void test_SharingToFacebook(int STLBDataID,
		...
		if(status.equalsIgnoreCase("PASS"))
		{
			...
			System.out.println(" ST_DS_01 : " +status);
			comLib.stLogResult(status, "ST_DS_01", "", red);

			/* verifying redirection from 'Get Sharethis' CTA button */
			comLib.stClickAndVerifyLink(OAUTH_WIDGET_GET_SHARETHIS_CTA_BUTTON, 16, 0, 0, "STOP");
			System.out.println(" ST_DS_03 : " +status);
			//status="PASS";
			comLib.stLogResult("PASS", "ST_DS_03", "", red);
			
			/* Verifying 'Shared To' Text on Done screen */
			status=stDoneScreen.stVerifySharedToText("FB", 1, "");
			System.out.println(" ST_DS_04 : " +status);
		...
		System.out.println(" ST_STM_02 : " +status);
		
		comLib.stLogResult(status, "ST_STM_02", "STOP", red);		
