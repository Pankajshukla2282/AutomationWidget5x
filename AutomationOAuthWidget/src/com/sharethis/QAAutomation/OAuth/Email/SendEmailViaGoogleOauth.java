...
public class SendEmailViaGoogleOauth {
	StringBuffer red = new StringBuffer("1");
	
	@Parameters( { "STLBDataID", "STLBExpVal", "STLDataID", "STLExpVal", "STBCDataID", "STEDataID"})
	@Test
public  void test_SendEmailViaGoogleOauth(int STLBDataID, int STLBExpVal, int STLDataID, 
		...
		/***********************************************************************
		 * ST_ESM_38- Email sharing using Google service
		 1. Browse to sharethis.com or any publisher site having widget 5x
		 2. Launch Widget 5x
		 3. Click on Email chicklet
		 4. Select Google Oauth service
		 5. Login with valid credentials
		 **********************************************************************/
		//***********Verification of Google Oauth service**********		
		...
		//*******Click on Google Oauth service*********
		comLib.stClickAndVerifyLink(EMAIL_GOOGLE_OAUTH, STBCDataID, 0, 1, "STOP");
		
		//*******Login With Google Account**********
		login.stGoogleLoginEmailScreen(STLDataID, STLExpVal, "STOP");		
		...
		comLib.stLogResult(status, "ST_ESM_38", "", red);		
