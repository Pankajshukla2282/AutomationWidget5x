...
public class STEmail {
    ...
    {			        	
        ...
        String msgbody = browser.getValue(emailMessageBox);
        System.out.println(msgbody);
		
		browser.type(emailMessageBox, msgbody + message + stFormat);
		try {
			Thread.sleep(3000);
		} catch (InterruptedException e) {
			e.printStackTrace();
	...
	// ********************[ stComposeEmailForGmailEmailService function Start ]*************************
	...
	String emailSubjectField="";
	String emailMessageBox="";
		
 	try {
 		
 	 	xPath.add(SIGNIN_GOOGLE_TO_FIELD);
 	 	errorMsg.add("To field is not present");
 	 		
 	 	xPath.add(SIGNIN_GOOGLE_MESSAGE_BODY);
 	 	errorMsg.add("Message field is not present");
 	 		
 	 	xPath.add(SIGNIN_GOOGLE_SUBJECT_FIELD);
 	 	errorMsg.add("Cancel button is not present");
 	 	
 	 	xPath.add(SIGNIN_GOOGLE_SEND_BUTTON);
    ...
    {			        	
    	...
    	if(!"".equals(to))
    	{
    		browser.focus(SIGNIN_GOOGLE_TO_FIELD);
			browser.click(SIGNIN_GOOGLE_TO_FIELD);
			try {
				Thread.sleep(3000);
			} catch (InterruptedException e) {
				e.printStackTrace();
    		...
    		browser.type(SIGNIN_GOOGLE_TO_FIELD, to );
	...
	returnVal=STFunctionLibrary.stRetValDes(ExpVal, actVal, "stComposeEmailForGmailEmailService",flow, hm);
	if(flow.contains("STOP")){
		assertEquals("PASS",returnVal);
	}
	return returnVal; 	
	...
	// ********************[ stComposeEmailForGmailEmailService function End ]*************************
	
	// ********************[ stComposeEmailForYahooEmailService function Start ]*************************
	...
	String emailMessageBox="";
 	...
 	try {
 	 	 	...
 	 	 	xPath.add(SIGNIN_YAHOO_TO_FIELD);
 	 	 	errorMsg.add("To field is not present");
 	 	 		
 	 	 	xPath.add(SIGNIN_YAHOO_MESSAGE_BODY);
 	 	 	errorMsg.add("Message field is not present");
 	 	 		
 	 	 	xPath.add(SIGNIN_YAHOO_SUBJECT_FIELD);
 	 	 	errorMsg.add("Cancel button is not present");
 	 	 	
 	 	 	xPath.add(SIGNIN_YAHOO_SEND_BUTTON);
 	 	 	errorMsg.add("Share Button is not present"); 		
 	    ...
 	    {			        	
 	    	...
 	    	if(!"".equals(to))
 	    	{
 	    		browser.focus(SIGNIN_YAHOO_TO_FIELD);
 				browser.click(SIGNIN_YAHOO_TO_FIELD);
 				try {
 					Thread.sleep(3000);
 				} catch (InterruptedException e) {
 					e.printStackTrace();
 	    		...
 	    		browser.type(SIGNIN_YAHOO_TO_FIELD, to );
 	        ...
 	        if(emailSubjectField.contains(subject) && emailMessageBox.contains(message))
 	        {
 	        	...
 	        	System.out.println("Values matched");
 	        	browser.click(SIGNIN_YAHOO_SEND_BUTTON);
 	        	break Block;
 	        }
 	        else
 	        {
 	        	actVal= 1; /*Failed to Compose email for Yahoo service.*/
 	        	System.out.println("Values Not matched");
 	        	break Block;
 	        }         
	...
	returnVal=STFunctionLibrary.stRetValDes(ExpVal, actVal, "stComposeEmailForYahooEmailService",flow, hm);
	if(flow.contains("STOP")){
		assertEquals("PASS",returnVal);
	}
	return returnVal; 	
	...
	// ********************[ stComposeEmailForYahooEmailService function End ]*************************
	
	// ********************[ stComposeEmailForOutlookEmailService function Start ]*************************
	
	public String stComposeEmailForOutlookEmailService(int dataId,int ExpVal,String flow )
	{		
		int actVal=1000;
		String returnVal=null;
		hm.clear();
	...
	String emailMessageBox="";
 	...
 	try {
 	 	 	...
 	 	 	xPath.add(SIGNIN_OUTLOOK_TO_FIELD);
 	 	 	errorMsg.add("To field is not present");
 	 	 		
 	 	 	xPath.add(SIGNIN_OUTLOOK_MESSAGE_BODY);
 	 	 	errorMsg.add("Message field is not present");
 	 	 		
 	 	 	xPath.add(SIGNIN_OUTLOOK_SUBJECT_FIELD);
 	 	 	errorMsg.add("Cancel button is not present");
 	 	 	
 	 	 	xPath.add(SIGNIN_OUTLOOK_SEND_BUTTON);
 	 	 	errorMsg.add("Send Button is not present"); 		
 	 		...
 	 		emailSubjectField = browser.getValue(SIGNIN_OUTLOOK_SUBJECT_FIELD);
 	 		System.out.println(emailSubjectField);
 	 		emailMessageBox = browser.getText(SIGNIN_OUTLOOK_MESSAGE_BODY);
 	 		System.out.println(emailMessageBox);
 	    ...
 	    {			        	
 	    	...
 	    	if(!"".equals(to))
 	    	{
 	    		browser.focus(SIGNIN_OUTLOOK_TO_FIELD);
// 				browser.click(SIGNIN_OUTLOOK_TO_FIELD);
 				try {
 					Thread.sleep(3000);
 				} catch (InterruptedException e) {
 					e.printStackTrace();
 	    		...
 	    		browser.type(SIGNIN_OUTLOOK_TO_FIELD, to );
 	        ...
 	        {
 	        	actVal= 0; /*Email is Composed successfully for Outlook service */
 	        	System.out.println("Values matched");
 	        	browser.click(SIGNIN_OUTLOOK_SEND_BUTTON);
 	        	break Block;
 	        }
 	        else
 	        {
	...
	returnVal=STFunctionLibrary.stRetValDes(ExpVal, actVal, "stComposeEmailForOutlookEmailService",flow, hm);
	if(flow.contains("STOP")){
		assertEquals("PASS",returnVal);
	}
	return returnVal; 	
